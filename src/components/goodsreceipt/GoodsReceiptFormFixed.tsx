...
export function GoodsReceiptForm({ receipt, viewMode = false, onSuccess, onCancel }: GoodsReceiptFormProps) {
  ...
  const onSubmit = (data: GoodsReceiptFormData, isDraft: boolean = true) => {
    ...
    const newReceipt: GoodsReceipt = {
      ...
      partner_code: data.partner_id ? mockPartners.find(p => p.id === data.partner_id)?.partner_code : undefined,
      partner_name: data.partner_id ? mockPartners.find(p => p.id === data.partner_id)?.partner_name : undefined,
              ...
              {receipt?.lines.map((line) => (
                <Card key={line.id} className="p-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    <div>
                      <Label className="text-xs text-muted-foreground">{t.assetModel}</Label>
                      <div>
                    ...
                    <div>
                      <Label className="text-xs text-muted-foreground">{t.unitOfMeasure}</Label>
                      <div>
                        <Badge variant="outline">{line.uom_code}</Badge>
                      </div>
                    </div>
                    <div>
                      <Label className="text-xs text-muted-foreground">{t.trackingType}</Label>
                      <div>
                    ...
                    <div>
                      <Label className="text-xs text-muted-foreground">{t.plannedQty}</Label>
    ...
    <form onSubmit={handleSubmit((data) => onSubmit(data, false))} className="space-y-6">
      ...
      <Card>
        ...
        <CardContent>
          {fields.length === 0 ? (
            ...
            <div className="space-y-4">
              {fields.map((field, index) => {
                const selectedModel = watch(`lines.${index}.model_id`) ? 
                  mockModelAssets.find(m => m.id === watch(`lines.${index}.model_id`)) : null
                ...
                return (
                  <Card key={field.id} className="p-4 border-l-4 border-l-blue-500">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {selectedModel && (
                          <Badge variant="secondary" className="text-xs">
                            {t[selectedModel.tracking_type as keyof typeof t]}
                          </Badge>
                        )}
                      </div>
                      <Button
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                      ...
                      <div className="space-y-2">
                        <Label className="text-sm">{t.assetModel} *</Label>
                      ...
                      {/* Tracking Type (Auto-filled, Disabled) */}
                      <div className="space-y-2">
                        <div className="flex items-center gap-2">
                          <Label className="text-sm">{t.trackingType}</Label>
                          <TooltipProvider>
                            <Tooltip>
                              <TooltipTrigger>
                              ...
                              <TooltipContent>
                                <p className="max-w-xs text-xs">{t.trackingTooltip}</p>
                              </TooltipContent>
                            </Tooltip>
                          </TooltipProvider>
                        </div>
                        <Input
                          value={selectedModel ? t[selectedModel.tracking_type as keyof typeof t] : t.None}
                          disabled
                          className="bg-muted"
                        />
                        <input
                          type="hidden"
                          {...register(`lines.${index}.tracking_type`)}
                          value={watch(`lines.${index}.tracking_type`)}
                        />
                      </div>

                      {/* Planned Quantity */}
                      <div className="space-y-2">
                        <Label className="text-sm">{t.plannedQty} *</Label>
                        <Input
                          type="number"
                          min="1"
                          step="1"
          ...
          {/* Add Line Button - Positioned Below Lines */}
      ...
      <div className="flex justify-end gap-3">
        ...
        <Button type="button" onClick={handleSubmit((data) => onSubmit(data, true))} variant="secondary">
